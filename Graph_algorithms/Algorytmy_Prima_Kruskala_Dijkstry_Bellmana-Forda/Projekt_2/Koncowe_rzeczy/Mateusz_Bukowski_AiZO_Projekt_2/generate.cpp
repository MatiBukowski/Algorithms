...
//	
	...
	gen_adj_list = new Gen_Adj_List(incidence_matrix, n, density_edges, weight_edges);
	gen_adj_list->generate_adjacency_list();
}

bool Generate::check_edge_exist(int a, int b, int n, int *Unew, int *Vnew) {
	for(int i=0; i<n; i++) {
...
void Generate::generate_incidence_matrix() {
...
int* Generate::getVertex() {
	return vertex;
}

int* Generate::getWeight() {
	return weight_edges;
}

int** Generate::getIncidenceMatrix() {
...
int Generate::getDensityEdges() {
	return density_edges;
}

int** Generate::get_Adj_List() {
