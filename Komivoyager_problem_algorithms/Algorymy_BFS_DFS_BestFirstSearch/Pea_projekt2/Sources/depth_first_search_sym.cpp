...
Depth_first_search_sym::Depth_first_search_sym(int *Sym_array, int Cities_number) {
    sym_array = Sym_array;
    cities_number = Cities_number;
}

Depth_first_search_sym::~Depth_first_search_sym() {}

int Depth_first_search_sym::search_bound(int current_city, int distance, bool visited[], int first_city) {                     // funkcja obliczająca dolną granicę dla częściowej trasy, distance - koszt dotychczas przebytej drogi
...
void Depth_first_search_sym::DFS_algorithm() {
    ...
    cout << "Algorytm Depth First Search (SYMETRYCZNA) - czas: " << elapsedTime1*1e3 << " ms"<< endl << endl;

    ofstream results_file;                                                                                      // zapisywanie wyników do pliku
    results_file.open("DFS_SYM_results_PEA2.csv", ios::app);

    results_file << "Algorytm Depth First Search (SYMETRYCZNA); " << cities_number << " miast; " << "Długość ścieżki: " << final_distance << "; "<< "Czas: " << elapsedTime1*1e3 << " ms" << endl;
    results_file.close();
...
Node** Depth_first_search_sym::add_to_array(Node** array, Node* node, int &nodes_amount) {
...
Node** Depth_first_search_sym::remove_top_from_array(Node** array, int &nodes_amount) {
...
double Depth_first_search_sym::getElapsedTime() {
    return elapsedTime1;
}
