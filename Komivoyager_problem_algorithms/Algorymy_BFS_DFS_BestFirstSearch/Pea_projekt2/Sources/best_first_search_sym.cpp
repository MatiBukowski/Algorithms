...
Best_first_search_sym::Best_first_search_sym(int *Sym_array, int Cities_number) {
    sym_array = Sym_array;
    cities_number = Cities_number;
}

Best_first_search_sym::~Best_first_search_sym() {}

int Best_first_search_sym::search_bound(int current_city, int distance, bool visited[], int first_city) {                     // funkcja obliczająca dolną granicę dla częściowej trasy, distance - koszt dotychczas przebytej drogi
    ...
    for(int i=0; i<cities_number; i++) {                                                    // szukanie najkrotszej mozliwej sciezki wyjscia z obecnego miasta
        if(current_city > i) {
            index = ((current_city - 1)*current_city/2)+i;                                  // obliczanie indeksu tablic sym_array
        } else {
    ...
    for(int i=0; i<cities_number; i++) {                                                    // szukanie najkrotszej mozliwej sciezki wyjscia z kolejnych miast
        if(!visited[i]) {                                                                   // jeśli miasto nei jest odwiedzone
            ...
            for(int j=0; j<cities_number; j++) {
                if(i > j) {                                                                 // obliczanie indeksu tablic sym_array
                    index = ((i - 1)*i)/2+j;
                } else {
                    index = ((j - 1)*j)/2+i;
                }
...
void Best_first_search_sym::BFS_algorithm() {
    ...
    final_distance = array[0]->getDistance() + sym_array[index];                 // odległość do ostatniego miasta + odległość od tego miasta do miasta początkowego
    ...
    cout << "Algorytm Best First Search (SYMETRYCZNA) - czas: " << elapsedTime1*1e3 << " ms"<< endl << endl;

    ofstream results_file;
    results_file.open("Best_SYM_results_PEA2.csv", ios::app);                                                                                // zapisywanie wyników do pliku

    results_file << "Algorytm Best First Search (SYMETRYCZNA); " << cities_number << " miast; " << "Długość ścieżki: " << final_distance << "; "<< "Czas: " << elapsedTime1*1e3 << " ms" << endl;
    results_file.close();
...
void Best_first_search_sym::heap_making(Node** array, int size) {                     // funckja tworząca kopiec
...
void Best_first_search_sym::heap(Node** array, int size, int parentIndex) {       // funkcja przywracająca kopiec po dodaniu elementu
...
Node** Best_first_search_sym::add_to_array(Node** array, Node* node, int &nodes_amount) {
...
Node** Best_first_search_sym::remove_best_from_array(Node** array, int &nodes_amount) {
...
double Best_first_search_sym::getElapsedTime() {
    return elapsedTime1;
}
